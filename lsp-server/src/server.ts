...
import {
  findLineNumberDefinitionRange,
  findLineNumberDefinitionRangeFromTokens,
  findLineNumberReferenceRangeFromTokens,
    ...
    documentSettings.set(
      ...
      connection.workspace
        ...
        .then((settings) => ({
          ...
          lineNumberIncrement:
            settings?.lineNumberIncrement ??
            defaultSettings.lineNumberIncrement,
          maxLineLength:
            settings?.maxLineLength ?? defaultSettings.maxLineLength,
          uppercaseKeywords:
            settings?.uppercaseKeywords ?? defaultSettings.uppercaseKeywords,
          trace: {
            server: settings?.trace?.server ?? defaultSettings.trace.server,
          },
          logging: {
            level: settings?.logging?.level ?? defaultSettings.logging.level,
          },
        })),
    );
  }
...
connection.onInitialized(() => {
  ...
  connection.window.showInformationMessage(
    "ZX BASIC language server initialized and ready.",
  );
});
...
async function validateTextDocument(
  ...
  const unmatchedNextStatements: Array<{
    line: number;
    start: number;
    end: number;
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === TokenType.KEYWORD && token.value === "FOR") {
      ...
      for (let j = i + 1; j < tokens.length && j < i + 5; j++) {
        ...
        if (
          tokens[j].type === TokenType.STATEMENT_SEPARATOR ||
            ...
            diagnostics.push({
              ...
              range: {
                start: { line: token.line, character: token.start },
                  ...
                  line: token.line,
                  character: token.end + (nextVar ? nextVar.length + 1 : 0),
                },
              },
    ...
    if (token.type === TokenType.KEYWORD && token.value === "DIM") {
      ...
      for (let j = i + 1; j < tokens.length && j < i + 20; j++) {
        if (tokens[j].type === TokenType.IDENTIFIER) {
          ...
          tokens[j].type === TokenType.PUNCTUATION &&
          tokens[j].value === "("
        ) {
          parenStart = j;
        } else if (
      ...
      if (parenStart !== -1 && parenEnd !== -1) {
        ...
        const dimensionType = isStringArray
          ? "dimensions + string length"
          : "dimensions";
          ...
          diagnostics.push({
            ...
            range: {
              start: { line: token.line, character: token.start },
                ...
                line: tokens[parenEnd].line,
                character: tokens[parenEnd].end,
              },
            },
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (
      ...
      colorKeywords.includes(token.value)
    ) {
      if (
        (token.value === "INK" || token.value === "PAPER") &&
        isDrawingAttribute(tokens, i)
      ) {
        continue;
      }
  ...
  const dimDeclarations = new Map<
    string,
    { line: number; dimensions: number; isString: boolean }
  >();
  const arrayUsages = new Map<
    string,
    Array<{ line: number; usedDimensions: number; isString: boolean }>
  >();
      ...
      while (
        ...
        if (idToken.type === TokenType.IDENTIFIER) {
          ...
          if (i < tokens.length && tokens[i].value === "(") {
            ...
            const declaredDimensions = isStringArray
              ? Math.max(0, dimensionCount - 1)
              : dimensionCount;
      ...
      usages.forEach((usage) => {
        ...
        if (usedDims !== declaredDims && !isValidStringSlicing) {
          ...
          if (declaration.isString && usedDims === declaredDims + 1) {
            ...
            message +=
              " (extra parameter may be character position for string slicing)";
...
function detectCompletionContext(
  ...
  const keywordPattern =
    /\b(IF|FOR|PRINT|INPUT|LET|DIM|GOTO|GOSUB|READ|DATA|RETURN|NEXT|ELSE)\b/gi;
  ...
  const expressionContextKeywords = [
    "PRINT",
    "INPUT",
    "IF",
    "LET",
  ...
  for (const keyword of expressionContextKeywords) {
    if (linePrefix.toUpperCase().includes(keyword)) {
      ...
      const afterKeywordText = linePrefix.substring(
        linePrefix.toUpperCase().lastIndexOf(keyword) + keyword.length,
      );
...
connection.onCompletion(
  async (params: TextDocumentPositionParams): Promise<CompletionItem[]> => {
    ...
    if (context.isAfterLineNumberKeyword && /^\d*$/.test(currentWord)) {
      ...
      const sortedLineNumbers = Array.from(lineNumbers).sort(
    ...
    if (context.isAtStatementStart) {
      ...
      filteredKeywords = filteredKeywords.filter(
        (kw) => statementKeywords.has(kw.toUpperCase()) || isFunction(kw),
      );
      ...
      filteredKeywords = filteredKeywords.filter((kw) => {
        ...
        return (
          isFunction(kw) ||
          expressionKeywords.has(upper) ||
          !statementKeywords.has(upper)
        );
      });
    }
...
connection.onHover((params: HoverParams): Hover => {
  ...
  while (
    ...
    /[A-Za-z0-9_$%]/.test(fullLineText[endChar])
  ) {
    endChar++;
  }
  ...
  connection.console.log(
    `Hover debug: position=${position.line}:${position.character}, word="${word}", startChar=${startChar}, endChar=${endChar}`,
  );
  ...
  // Check if it's a variable (provide semantic information)
  if (word.match(/^[A-Za-z][A-Za-z0-9_]*[$%]?$/)) {
    // This looks like a variable - provide semantic analysis
    const variableInfo = analyzeVariableUsage(document, word, position);
    if (variableInfo) {
      return {
        contents: {
          kind: "markdown",
  ...
  if (/^\d+$/.test(word)) {
    ...
    const isTargetNumber =
      /\b(GOTO|GO TO|GOSUB|GO SUB|RUN|LIST|RESTORE|THEN|ELSE)\s*$/.test(
        beforeWord,
      );
    ...
    if (isTargetNumber) {
      ...
      for (let i = 0; i < allTokens.length; i++) {
        if (
          allTokens[i].type === TokenType.LINE_NUMBER &&
          allTokens[i].value === lineNum
        ) {
          foundLine = true;
          lineStartIdx = i;
          ...
          while (i < allTokens.length) {
            if (
              allTokens[i].type === TokenType.LINE_NUMBER ||
              allTokens[i].type === TokenType.EOF
            ) {
              lineEndIdx = i;
              break;
            }
      ...
      if (foundLine && lineStartIdx >= 0) {
        ...
        for (let j = lineStartIdx + 1; j < allTokens.length; j++) {
          if (
            allTokens[j].type === TokenType.LINE_NUMBER &&
            allTokens[j].line > startToken.line
          ) {
...
function getFunctionCompletions(prefix: string): CompletionItem[] {
  const functions = [
    ...
    },
    {
      ...
      detail: "Convert number to string",
    },
    {
      name: "CHR$",
      signature: "CHR$(code)",
      detail: "ASCII character from code",
    },
    {
      name: "CODE",
      signature: "CODE(string)",
      detail: "ASCII code of first character",
    },
...
function getHoverDocumentation(keyword: string): string {
  const hoverDocs: { [key: string]: string } = {
    ...
    CLEAR:
      "Clear variables and set memory limit\n\n**Syntax:**\n```\nCLEAR [address]\n```",
    ...
    VERIFY: 'Verify saved data\n\n**Syntax:**\n```\nVERIFY \"filename\"\n```',
    MERGE:
      'Merge program from tape\n\n**Syntax:**\n```\nMERGE \"filename\"\n```',
    LIST: "List program lines\n\n**Syntax:**\n```\nLIST [start] [, end]\n```",
    LLIST:
      "List to printer (128K)\n\n**Syntax:**\n```\nLLIST [start] [, end]\n```",
    LPRINT:
      "Print to printer (128K)\n\n**Syntax:**\n```\nLPRINT expression\n```",
    COPY: "Copy screen to printer\n\n**Syntax:**\n```\nCOPY\n```",
    ...
    SCREEN$:
      "Return character at screen position\n\n**Syntax:**\n```\nSCREEN$(line, column) -> string\n```",
    ...
    FORMAT:
      'Format disk (128K/Interface 1)\n\n**Syntax:**\n```\nFORMAT \"name\"\n```',
    MOVE: 'Move file (128K)\n\n**Syntax:**\n```\nMOVE \"from\" TO \"to\"\n```',
  };
  return hoverDocs[keyword] || "";
}

function getFunctionSignatureInfo(
  functionName: string,
  ...
  const functionSignatures: { [key: string]: SignatureInformation } = {
    ...
    LEN: {
      ...
      parameters: [{ label: "str", documentation: "The input string" }],
    },
    VAL: {
    ...
    STR$: {
      ...
      parameters: [{ label: "num", documentation: "The number to convert" }],
    },
    CHR$: {
      label: "CHR$(code: number): string",
      documentation: "Return the character represented by an ASCII code",
      parameters: [{ label: "code", documentation: "ASCII code (0-255)" }],
    },
    CODE: {
      ...
      parameters: [
        {
          label: "colour",
          documentation:
            "Color code 0=black, 1=blue, 2=red, 3=magenta, 4=green, 5=cyan, 6=yellow, 7=white",
        },
      ],
    },
    PAPER: {
    ...
    BORDER: {
      ...
      parameters: [{ label: "colour", documentation: "Color code" }],
    },
    PRINT: {
      ...
      parameters: [
        {
          label: "expression",
          documentation: "String or numeric expression to print",
        },
      ],
    },
    INPUT: {
    ...
    ABS: {
      ...
      parameters: [{ label: "num", documentation: "The number" }],
    },
    INT: {
    ...
    DATA: {
      ...
      parameters: [{ label: "constant", documentation: "Data value" }],
    },
    GOTO: {
    ...
    ATN: {
      ...
      parameters: [{ label: "x", documentation: "Any number" }],
    },
    SGN: {
    ...
    EXP: {
      ...
      parameters: [{ label: "x", documentation: "Exponent" }],
    },
    LN: {
    ...
    IN: {
      ...
      parameters: [{ label: "port", documentation: "Port number (0-65535)" }],
    },
    OUT: {
    ...
    PAUSE: {
      ...
      parameters: [
        {
          label: "duration",
          documentation: "Duration in 1/50ths second (0=wait for key)",
        },
      ],
    },
    RANDOMIZE: {
      ...
      parameters: [{ label: "seed", documentation: "Random seed (optional)" }],
    },
    RESTORE: {
      ...
      parameters: [
        {
          label: "line_number",
          documentation: "Line to restore from (optional)",
        },
      ],
    },
    SAVE: {
      ...
      parameters: [{ label: "filename", documentation: "File name" }],
    },
    LOAD: {
      label: 'LOAD "filename"',
...
// Analyze variable usage and provide hover information
function analyzeVariableUsage(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === TokenType.IDENTIFIER) {
      const currentVarName = token.value.replace(/[$%]$/, "").toUpperCase();

      if (currentVarName === baseName) {
        ...
        if (i > 0) {
          ...
          if (prevToken.type === TokenType.KEYWORD) {
            ...
            if (["LET", "DIM", "INPUT", "READ"].includes(prevKeyword)) {
              isDeclaration = true;
              context = prevKeyword;
            }
          }
  ...
  hoverInfo += `📍 **Used ${occurrences.length} time${occurrences.length !== 1 ? "s" : ""}**\n\n`;
...
function getCommandSignatureInfo(
  commandName: string,
): SignatureInformation | undefined {
  const commandSignatures: { [key: string]: SignatureInformation } = {
    PRINT: {
      ...
      documentation:
        "Print text or expressions to the screen. Use ; for no spacing, , for spacing",
      parameters: [
        {
          label: "expression",
          documentation: "String or numeric expression to print",
    ...
    DIM: {
      ...
      parameters: [
        ...
        {
          label: "size",
          documentation: "Dimension size (can be 1, 2, or 3 dimensions)",
        },
      ],
    },
    IF: {
    ...
    INK: {
      ...
      parameters: [{ label: "colour", documentation: "Colour code" }],
    },
    PAPER: {
      label: "PAPER colour",
  ...
  (params: DocumentSymbolParams): DocumentSymbol[] => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === TokenType.LINE_NUMBER) {
        ...
        token.type === TokenType.KEYWORD &&
        token.value.toUpperCase() === "DEF FN"
      ) {
    ...
    lineNumbers.forEach((info, lineNum) => {
      ...
      const symbolKind = info.hasGosub
        ? SymbolKind.Function
        : SymbolKind.Number;
    ...
    const variablesToShow = Array.from(variableDefinitions.entries()).slice(
      0,
      20,
    );
...
connection.onTypeDefinition((params: TypeDefinitionParams): Location | null => {
  ...
  const lexer = new ZXBasicLexer();
  const tokens = lexer.tokenize(text);
  const referenceRange = findLineNumberReferenceRangeFromTokens(
    tokens,
    params.position,
  );
  if (referenceRange) {
...
connection.onImplementation((params: ImplementationParams): Location | null => {
  ...
  if (context?.isLineNumber) {
    const definitionRange = findLineNumberDefinitionRangeFromTokens(
      tokens,
      context.oldName,
    );
    if (definitionRange) {
...
connection.onDefinition((params: DefinitionParams): Location | null => {
  ...
  const lexer = new ZXBasicLexer();
  const tokens = lexer.tokenize(text);
  const lineRange = findLineNumberReferenceRangeFromTokens(
    tokens,
    params.position,
  );
  if (lineRange) {
...
connection.onReferences((params: ReferenceParams): Location[] => {
  ...
  if (context && !context.isLineNumber) {
    ...
    const identifierRanges = findIdentifierReferenceRanges(
      tokens,
      context.oldName,
    );
  ...
  const referencingTokens = targetLineNum
    ? (referenceMap.get(targetLineNum) ?? [])
    : [];
  ...
  for (const token of tokens) {
    ...
    lenses.push({
      ...
      command: {
        ...
        arguments: [params.textDocument.uri, definitionRange.start, locations],
      },
    });
  }
...
connection.onCodeAction((params: CodeActionParams): CodeAction[] => {
  ...
  if (lines.length > 0 && lines[0].trim() && !lines[0].trim().match(/^\d+/)) {
    actions.push({
      ...
      edit: {
        changes: {
          [params.textDocument.uri]: addLineNumbers(
            lines,
  ...
  if (usedArrays.size > 0) {
    ...
    const dimStatement = undeclaredArrayNames
      .map((name) => `${name}(10)`)
      .join(", ");
...
connection.onDocumentFormatting(
  (params: DocumentFormattingParams): TextEdit[] => {
    ...
    if (!document) {
      connection.console.log(
        `[format] No document found for ${params.textDocument.uri}`,
      );
      return [];
    }

    connection.console.log(
      `[format] Received format request for ${params.textDocument.uri}`,
    );
    const text = document.getText();
    const lexer = new ZXBasicLexer();
    const tokens = lexer.tokenize(text);
    ...
    connection.console.log(
      `[format] autoRenumberLines produced ${renumberEdits.length} edits for ${params.textDocument.uri}`,
    );
  ...
  async (params: SemanticTokensParams): Promise<SemanticTokens> => {
    ...
    for (const token of tokens) {
      if (token.type === TokenType.LINE_NUMBER) {
        ...
        token.type === TokenType.KEYWORD &&
        (token.value.toUpperCase() === "INPUT" ||
          token.value.toUpperCase() === "READ")
      ) {
    ...
    // Generate semantic tokens - HYBRID APPROACH
    // Only provide semantic information that TextMate can't handle
    for (const token of tokens) {
      ...
      // Focus only on identifiers (variables) - this is where LSP adds the most value
      // Let TextMate handle everything else for consistency and performance

      // Only process identifiers (variables) - this is where LSP adds value
      if (token.type === TokenType.IDENTIFIER) {
        const varName = token.value.replace(/[$%]$/, "");
...
connection.languages.semanticTokens.onRange(
  async (params: SemanticTokensRangeParams): Promise<SemanticTokens> => {
    const document = documents.get(params.textDocument.uri);
    if (!document) {
      return { data: [] };
    }
...
export function getFoldingRanges(
  ...
  if (settings.logging.level === "debug") {
    connection.console.log(
      `Folding ranges debug: Analyzing ${lines.length} lines of text`,
    );
  }
    ...
    if (lineNumMatch) {
      ...
      if (/\b(GOSUB|GO\s+SUB)\s+(\d+)/i.test(line)) {
        ...
        if (targetMatch) {
          ...
          if (settings.logging.level === "debug") {
            connection.console.log(
              `Folding ranges debug: Found GOSUB target line ${targetMatch[2]} on line ${i}`,
            );
          }
        }
      }
  ...
  if (settings.logging.level === "debug") {
    connection.console.log(
      `Folding ranges debug: Found ${gosubTargets.size} GOSUB target lines: ${[...gosubTargets].join(", ")}`,
    );
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/\bFOR\s+/i.test(upperLine)) {
      ...
      if (settings.logging.level === "debug") {
        connection.console.log(
          `Folding ranges debug: Found FOR statement on line ${i}`,
        );
      }
      ...
      if (forStack.length > 0) {
        ...
        if (forLoop) {
          ...
          if (settings.logging.level === "debug") {
            connection.console.log(
              `Folding ranges debug: Created FOR loop folding range: lines ${forLoop.startLine}-${i}`,
            );
          }
        }
      } else if (settings.logging.level === "debug") {
        connection.console.log(
          `Folding ranges debug: Found NEXT statement on line ${i} without matching FOR`,
        );
      }
    }
    ...
    if (lineNum && gosubTargets.has(lineNum)) {
      ...
      if (settings.logging.level === "debug") {
        connection.console.log(
          `Folding ranges debug: Found subroutine target on line ${i} (line number ${lineNum})`,
        );
      }
    }
  }
  ...
  for (const subroutine of subroutines) {
    ...
    for (let i = subroutine.startLine + 1; i < lines.length; i++) {
      if (/\bRETURN\b/i.test(lines[i].toUpperCase())) {
        ...
        if (settings.logging.level === "debug") {
          connection.console.log(
            `Folding ranges debug: Found RETURN statement for subroutine ${subroutine.lineNumber} on line ${i}`,
          );
        }
        break;
      }
      ...
      if (nextLineMatch && gosubTargets.has(nextLineMatch[1])) {
        ...
        if (settings.logging.level === "debug") {
          connection.console.log(
            `Folding ranges debug: Stopped subroutine ${subroutine.lineNumber} at next subroutine target on line ${i}`,
          );
        }
        break;
      }
    ...
    if (endLine > subroutine.startLine) {
      ...
      if (settings.logging.level === "debug") {
        connection.console.log(
          `Folding ranges debug: Created subroutine folding range: lines ${subroutine.startLine}-${endLine} (subroutine ${subroutine.lineNumber})`,
        );
      }
    } else if (settings.logging.level === "debug") {
      connection.console.log(
        `Folding ranges debug: No RETURN found for subroutine ${subroutine.lineNumber}, skipping folding range`,
      );
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/\bDATA\b/i.test(upperLine)) {
      if (dataStart === null) {
        ...
        if (settings.logging.level === "debug") {
          connection.console.log(
            `Folding ranges debug: Started DATA block on line ${i}`,
          );
        }
      }
      lastDataLine = i;
      ...
      if (lastDataLine > dataStart) {
        ...
        if (settings.logging.level === "debug") {
          connection.console.log(
            `Folding ranges debug: Created DATA block folding range: lines ${dataStart}-${lastDataLine}`,
          );
        }
      }
      dataStart = null;
  ...
  if (dataStart !== null && lastDataLine > dataStart) {
    ...
    if (settings.logging.level === "debug") {
      connection.console.log(
        `Folding ranges debug: Created final DATA block folding range: lines ${dataStart}-${lastDataLine}`,
      );
    }
  }
  ...
  if (forStack.length > 0 && settings.logging.level === "debug") {
    connection.console.log(
      `Folding ranges debug: Warning: ${forStack.length} unmatched FOR statements remain in stack`,
    );
    forStack.forEach((forLoop, index) => {
      connection.console.log(
        `Folding ranges debug: Unmatched FOR ${index + 1}: started on line ${forLoop.startLine}`,
      );
    });
  }
  ...
  if (settings.logging.level === "debug") {
    connection.console.log(
      `Folding ranges debug: Returning ${foldingRanges.length} folding ranges`,
    );
    foldingRanges.forEach((range, index) => {
      connection.console.log(
        `Folding ranges debug: Range ${index + 1}: lines ${range.startLine}-${range.endLine}, kind: ${range.kind}`,
      );
    });
  }
...
connection.onFoldingRanges(
  async (params: FoldingRangeParams): Promise<FoldingRange[]> => {
    ...
    if (settings.logging.level === "debug") {
      connection.console.log(
        `Folding ranges debug: Processing folding ranges request for ${params.textDocument.uri}`,
      );
    }
...
connection.languages.callHierarchy.onPrepare(
  async (
    ...
    while (
      ...
      /[A-Za-z0-9_$%]/.test(lineText[wordEnd])
    ) {
      wordEnd++;
    }
    ...
    const item: CallHierarchyItem = {
      ...
      range: {
        start: position,
          ...
          line: position.line,
          character: position.character + lineNumber.length,
        },
      },
      selectionRange: {
        start: position,
  ...
  async (
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const gotoPattern = new RegExp(
        `\\b(GOSUB|GO\\s+SUB)\\s+${targetLineNum}\\b`,
        "gi",
      );
      let match;
    ...
    for (let i = subroutineStart; i < subroutineEnd && i < lines.length; i++) {
      ...
      while ((match = gotoPattern.exec(line)) !== null) {
        ...
        const calleeItem: CallHierarchyItem = {
          ...
          range: {
            start: {
              line: parseInt(targetLineNum) - 1 < 0 ? 0 : i,
              character: 0,
            },
            end: { line: i, character: line.length },
          },
