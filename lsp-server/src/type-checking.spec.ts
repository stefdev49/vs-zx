...
`;
  ...
  function buildVariableTypes(tokens: any[]): Map<string, 'string' | 'numeric' | 'unknown'> {
  ...
  function checkTypeMismatches(tokens: any[], variableTypes: Map<string, 'string' | 'numeric' | 'unknown'>): number {
  ...
  test('should infer variable types from INPUT statements', () => {
    ...
    expect(variableTypes.get('input_val')).toBe('numeric');
    expect(variableTypes.get('input_name')).toBe('string');
  });
  ...
  test('should detect type mismatches in numeric function calls', () => {
    ...
    expect(mismatchCount).toBe(2); // ABS(name$) and SQR(input_name$)
  });
