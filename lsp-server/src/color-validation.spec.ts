...
describe('Color Value Validation Tests', () => {
  ...
  function validateColorCommand(code: string): boolean {
    const tokens = lexer.tokenize(code).filter(t => t.type !== TokenType.EOF);

    // Find color keyword and following number (possibly with minus sign)
    ...
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type === TokenType.KEYWORD && ['INK', 'PAPER', 'BORDER'].includes(tokens[i].value)) {
        ...
        // Find next number, potentially preceded by minus
        let j = i + 1;
        ...
        if (j < tokens.length && tokens[j].type === TokenType.NUMBER) {
          colorValue = parseInt(tokens[j].value, 10) * (isNegative ? -1 : 1);
        }
        break;
      }
    }
  ...
  test.each(testCases)('$description', ({ code, isValid }) => {
    const result = validateColorCommand(code);
