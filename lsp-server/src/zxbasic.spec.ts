...
describe("ZXBasicLexer", () => {
  ...
  test("should handle line numbers and basic structure", () => {
    ...
    expect(
      tokens.some((t) => t.type === TokenType.KEYWORD && t.value === "PRINT"),
    ).toBe(true);
    expect(
      tokens.some((t) => t.type === TokenType.STRING && t.value === '"HELLO"'),
    ).toBe(true);
  });
  ...
  test("should handle invalid characters", () => {
    ...
    expect(
      tokens.some((t) => t.type === TokenType.INVALID && t.value === "@"),
    ).toBe(true);
  });
  ...
  test("should tokenize multi-statement lines with colon separators", () => {
    ...
    expect(
      tokens.filter(
        (t) => t.type === TokenType.STATEMENT_SEPARATOR && t.value === ":",
      ).length,
    ).toBe(2);
    expect(
      tokens.some((t) => t.type === TokenType.LINE_NUMBER && t.value === "10"),
    ).toBe(true);
    expect(
      tokens.some((t) => t.type === TokenType.KEYWORD && t.value === "LET"),
    ).toBe(true);
    expect(
      tokens.some((t) => t.type === TokenType.KEYWORD && t.value === "PRINT"),
    ).toBe(true);
    expect(
      tokens.some((t) => t.type === TokenType.KEYWORD && t.value === "GOTO"),
    ).toBe(true);
  });
  ...
  test("should handle two-word keyword DEF FN", () => {
    ...
    expect(
      tokens1.some((t) => t.type === TokenType.KEYWORD && t.value === "DEFFN"),
    ).toBe(true);
    expect(
      tokens1.filter((t) => t.value === "FN" && t.type === TokenType.KEYWORD)
        .length,
    ).toBe(0); // FN should be consumed by DEFFN
    ...
    expect(
      tokens3.some((t) => t.type === TokenType.KEYWORD && t.value === "FN"),
    ).toBe(true);
  });
...
describe("LSP Server Integration", () => {
  ...
  test("DIM declarations and usage: numeric array mismatch detection", () => {
    ...
    const dimIndex = tokens.findIndex(
      (t) => t.type === TokenType.KEYWORD && t.value === "DIM",
    );
  ...
  test("DIM declarations and usage: string array trailing length handling", () => {
    ...
    const dimId = tokens.find(
      (t) => t.type === TokenType.IDENTIFIER && t.value === "N$",
    );
    expect(dimId).toBeDefined();

    // Find parentheses and parameters after DIM
    const dimPos = tokens.findIndex(
      (t) => t.type === TokenType.KEYWORD && t.value === "DIM",
    );
    ...
    const numberInside = tokens.some(
      (t, idx) => idx > dimPos && t.type === TokenType.NUMBER,
    );
  ...
  describe("Completion Provider", () => {
    test("should return all keywords for empty input", () => {
      ...
      const isFuncResult = (global as any).isFunction
        ? (global as any).isFunction("SIN")
        : false;
    ...
    test("should identify functions correctly", () => {
      ...
      const testInput = "SIN";
      const expectedFunctions = functions.filter((f) =>
        f.toLowerCase().startsWith(testInput.toLowerCase()),
      );
    ...
    test("should get keyword documentation", () => {
      ...
      testKeywords.forEach((keyword) => {
        docs.push(
          keywords[keyword.toUpperCase() as keyof typeof keywords] ||
            "ZX BASIC keyword",
        );
      });
  ...
  describe("Signature Help Provider", () => {
    test("should provide signature information for functions", () => {
      ...
      expect(signatures[0]).toHaveProperty(
        "label",
        "SIN(angle: number): number",
      );
      expect(signatures[1]).toHaveProperty(
        "label",
...
describe("ZX BASIC Language Processing", () => {
  test("should process complete BASIC line", () => {
    ...
    expect(
      tokens.some(
        (t) => t.type === TokenType.STRING && t.value === '"VALUE IS"',
      ),
    ).toBe(true);
  ...
  test("should handle BASIC syntax correctly", () => {
    ...
    testCases.forEach((code) => {
      ...
      const invalidCount = tokens.filter(
        (t) => t.type === TokenType.INVALID,
      ).length;
